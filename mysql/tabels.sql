...
CREATE TABLE MESSAGES_INGREDIENTS
(
	ID_MESSG INT AUTO_INCREMENT,
	MESSAGE VARCHAR(300),
	PRIMARY KEY(ID_MESSG)
)
AUTO_INCREMENT = 6000;

DELIMITER $$

CREATE PROCEDURE add_ingredient(NAME VARCHAR(20), STOCK INT, INGR_DATE DATETIME)
BEGIN
	INSERT INTO INGREDIENTS (NAME, STOCK, INGREDIENT_DATE) VALUES (NAME, STOCK, INGR_DATE);
	COMMIT;
END $$

CREATE PROCEDURE update_ingredient(ID_INGR INT, STOCK_INGR INT, INGR_DATE DATETIME)
BEGIN
	DECLARE STOCK_INGREDIENT INT;
	SELECT STOCK INTO STOCK_INGREDIENT
	FROM INGREDIENTS
	WHERE ID_INGREDIENT = ID_INGR;

	UPDATE INGREDIENTS SET STOCK = STOCK_INGREDIENT + STOCK_INGR, INGREDIENT_DATE = INGR_DATE
	WHERE ID_INGREDIENT = ID_INGR;
	COMMIT;
END $$
...
BEGIN
	...
	SELECT ORDER_DATE INTO INGR_DATE
	FROM ORDERS
	WHERE ID_ORDER = NEW.ID_ORDER;
	...
	UPDATE INGREDIENTS SET STOCK = STOCK_INGR - QUANTITY_CMD, INGREDIENT_DATE = INGR_DATE
	WHERE ID_INGREDIENT = MAIN_INGR;
...
CREATE TRIGGER TGR_MESSAGES_INGREDIENTS_1
	AFTER INSERT ON INGREDIENTS
    FOR EACH ROW
BEGIN
	INSERT INTO MESSAGES_INGREDIENTS(MESSAGE) VALUES(CONCAT('S-a inserat un nou ingredient: ', NEW.ID_INGREDIENT));
END $$

CREATE TRIGGER TGR_MESSAGES_INGREDIENTS_2
	AFTER UPDATE ON INGREDIENTS
    FOR EACH ROW
BEGIN
	INSERT INTO MESSAGES_INGREDIENTS(MESSAGE) VALUES(CONCAT('S-a actualizat un nou ingredient: ', NEW.ID_INGREDIENT));
END $$
